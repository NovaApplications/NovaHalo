...
public class InvariantDeviceProfile {
    ...
    InvariantDeviceProfile(Context context, String n, float w, float h, int r, int c, int fr, int fc,
        ...
        minWidthDps = w;
        minHeightDps = h;
        numRows = Math.max(1, r);
        numColumns = Math.max(1, c);
        numFolderRows = Math.max(1, fr);
        numFolderColumns = Math.max(1, fc);
        iconSize = is;
        iconTextSize = its;
        numHotseatIcons = Math.max(1, hs);
        hotseatIconSize = his;
    ...
    InvariantDeviceProfile(Context context) {
        ...
        ArrayList<InvariantDeviceProfile> predefinedProfiles = getPredefinedDeviceProfiles(context);
        if (predefinedProfiles.isEmpty()) {
            throw new RuntimeException("No device profiles found in XML");
        }

        ArrayList<InvariantDeviceProfile> closestProfiles = findClosestDeviceProfiles(
                minWidthDps, minHeightDps, predefinedProfiles);
        InvariantDeviceProfile interpolatedDeviceProfileOut =
                invDistWeightedInterpolate(context, minWidthDps, minHeightDps, closestProfiles);

        InvariantDeviceProfile closestProfile = closestProfiles.get(0);
        numRows = Math.max(1, closestProfile.numRows);
        numColumns = Math.max(1, closestProfile.numColumns);
        numHotseatIcons = Math.max(1, closestProfile.numHotseatIcons);
        defaultLayoutId = closestProfile.defaultLayoutId;
        numFolderRows = Math.max(1, closestProfile.numFolderRows);
        numFolderColumns = Math.max(1, closestProfile.numFolderColumns);

        iconSize = Math.max(1, interpolatedDeviceProfileOut.iconSize);
        iconBitmapSize = Math.max(1, Utilities.pxFromDp(iconSize, dm));
        iconTextSize = Math.max(1, interpolatedDeviceProfileOut.iconTextSize);
        hotseatIconSize = Math.max(1, interpolatedDeviceProfileOut.hotseatIconSize);
        ...
        try (XmlResourceParser parser = context.getResources().getXml(R.xml.device_profiles)) {
            ...
            while (((type = parser.next()) != XmlPullParser.END_TAG ||
                ...
                if ((type == XmlPullParser.START_TAG) && "profile".equals(parser.getName())) {
                    ...
                    int numRows = a.getInt(R.styleable.InvariantDeviceProfile_numRows, 4);
                    int numColumns = a.getInt(R.styleable.InvariantDeviceProfile_numColumns, 4);
                    float iconSize = a.getFloat(R.styleable.InvariantDeviceProfile_iconSize, 48);
                    profiles.add(new InvariantDeviceProfile(context,
                            ...
                            a.getFloat(R.styleable.InvariantDeviceProfile_iconSize, iconSize),
                            a.getFloat(R.styleable.InvariantDeviceProfile_iconTextSize, 12),
