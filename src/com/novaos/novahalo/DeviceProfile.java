...
public class DeviceProfile {
    ...
    public Point getSearchBarDimensForWidgetOpts() {
        ...
        if (isTablet) {
            ...
            gap = ((width - 2 * edgeMarginPx
                    - (inv.numColumns * cellWidthPx)) / Math.max(1, (2 * (inv.numColumns + 1))))
                    + edgeMarginPx;
        } else {
    ...
    public void layout(Launcher launcher, boolean notifyListeners) {
        ...
        float workspaceCellWidth = (float) getCurrentWidth() / Math.max(1, inv.numColumns);
        float hotseatCellWidth = (float) getCurrentWidth() / Math.max(1, inv.numHotseatIcons);
